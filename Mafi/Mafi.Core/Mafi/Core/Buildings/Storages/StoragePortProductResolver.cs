...
// Type: Mafi.Core.Buildings.Storages.StoragePortProductResolver
...
namespace Mafi.Core.Buildings.Storages
{
  ...
  internal class StoragePortProductResolver : PortProductResolverBase<StorageBase>
  {
    private readonly Dict<ProductProto, ImmutableArray<ProductProto>> m_arraysCache;

    public StoragePortProductResolver(ProtosDb protosDb)
    {
      ...
      bool fallbackToUnlockedIfNoRecipesAssigned)
    {
      ...
      if (!this.m_arraysCache.TryGetValue(entity.StoredProduct.Value, out portProduct))
      {
        portProduct = ImmutableArray.Create<ProductProto>(entity.StoredProduct.Value);
        this.m_arraysCache.Add(entity.StoredProduct.Value, portProduct);
      }
      return portProduct;
    }
