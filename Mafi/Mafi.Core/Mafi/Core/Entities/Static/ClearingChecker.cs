// Decompiled with JetBrains decompiler
// Type: Mafi.Core.Entities.Static.ClearingChecker
// Assembly: Mafi.Core, Version=0.6.3.0, Culture=neutral, PublicKeyToken=null
// MVID: 2D94E377-7747-46A4-A766-4DF7ABFEBD30
// Assembly location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.Core.dll
// XML documentation location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.Core.xml

using eLxG93FZl5M3yHxFGb;
using Mafi.Core.Entities.Validators;

#nullable disable
namespace Mafi.Core.Entities.Static
{
  /// <summary>
  /// Validates whether a static entity can be successfully cleared before removal. This effectively prevents from
  /// deleting entities that are not ready for it e.g. last vehicle depot in the game or storage full of toxic waste.
  /// </summary>
  [GlobalDependency(RegistrationMode.AsAllInterfaces, false, false)]
  public class ClearingChecker : IEntityRemovalValidator<IStaticEntity>, IEntityRemovalValidator
  {
    public EntityValidatorPriority Priority => EntityValidatorPriority.Default;

    EntityValidationResult IEntityRemovalValidator<IStaticEntity>.CanRemove(
      IStaticEntity entity,
      EntityRemoveReason reason)
    {
      return reason == EntityRemoveReason.Collapse ? EntityValidationResult.Success : entity.CanStartDeconstruction();
    }

    public ClearingChecker()
    {
      MBiHIp97M4MqqbtZOh.rMWAw2OR8();
      // ISSUE: explicit constructor call
      base.\u002Ector();
    }
  }
}
