// Decompiled with JetBrains decompiler
// Type: Mafi.Unity.Ports.Io.IoPortVisual
// Assembly: Mafi.Unity, Version=0.6.3.0, Culture=neutral, PublicKeyToken=null
// MVID: E477F8BD-C838-4DE6-805D-A367452B274A
// Assembly location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.Unity.dll
// XML documentation location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.Unity.xml

using Mafi.Core.Ports.Io;
using ut12pZTdSUNA6wM24P;

#nullable disable
namespace Mafi.Unity.Ports.Io
{
  public readonly struct IoPortVisual
  {
    public readonly IoPortShapeProto ShapeProto;
    public readonly Tile3i Position;
    public readonly Direction903d Direction;
    public readonly bool IsEndPort;
    public readonly ColorRgba BlueprintColor;

    public IoPortVisual(
      Tile3i position,
      Direction903d direction,
      IoPortShapeProto shapeProto,
      bool isEndPort,
      ColorRgba blueprintColor)
    {
      xxhJUtQyC9HnIshc6H.OukgcisAbr();
      this.Position = position;
      this.Direction = direction;
      this.ShapeProto = shapeProto;
      this.IsEndPort = isEndPort;
      this.BlueprintColor = blueprintColor;
    }
  }
}
