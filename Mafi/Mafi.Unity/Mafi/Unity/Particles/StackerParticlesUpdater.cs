...
// Type: Mafi.Unity.Particles.StackerParticlesUpdater
...
namespace Mafi.Unity.Particles
{
  ...
  public class StackerParticlesUpdater : IEntityMbUpdater
  {
    private readonly Dict<IEntity, ImmutableArray<StackerParticlesController>> m_controllers;
    private readonly List<ParticleSystem> m_particleSystemsTmp;
    private readonly Lyst<StackerParticlesController> m_controllersTmp;

    public void AddOnSyncIfNeeded(EntityMb entityMb)
    {
      if (!(entityMb.Entity is Stacker entity))
        return;
      ...
      foreach (ParticlesParams particlesParam in entity.Prototype.Graphics.ParticlesParams)
      {
        ...
        if ((UnityEngine.Object) system == (UnityEngine.Object) null)
          ...
          this.m_controllersTmp.Add(new StackerParticlesController(entity, system, particlesParams));
      }
      this.m_particleSystemsTmp.Clear();
    ...
    {
      this.m_controllers.TryRemove(entityMb.Entity, out ImmutableArray<StackerParticlesController> _);
    }

    public void RenderUpdate(GameTime time)
    {
      foreach (ImmutableArray<StackerParticlesController> immutableArray in this.m_controllers.Values)
      {
        foreach (StackerParticlesController particlesController in immutableArray)
    ...
    public StackerParticlesUpdater()
    {
      xxhJUtQyC9HnIshc6H.OukgcisAbr();
      this.m_controllers = new Dict<IEntity, ImmutableArray<StackerParticlesController>>();
      this.m_particleSystemsTmp = new List<ParticleSystem>();
      this.m_controllersTmp = new Lyst<StackerParticlesController>();
