...
namespace Mafi
{
  public interface IEvent<T> : IEventNonSaveable<T>
  {
    /// <summary>
    ...
    /// </summary>
    void Add<TOwner>(TOwner owner, Action<T> action) where TOwner : class;

    void Remove<TOwner>(TOwner owner, Action<T> action) where TOwner : class;

    bool IsAdded<TOwner>(TOwner owner, Action<T> action) where TOwner : class;
  }
}
