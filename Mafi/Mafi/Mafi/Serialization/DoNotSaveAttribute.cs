// Decompiled with JetBrains decompiler
// Type: Mafi.Serialization.DoNotSaveAttribute
// Assembly: Mafi, Version=0.6.3.0, Culture=neutral, PublicKeyToken=null
// MVID: B4163D2E-70BA-4761-B75B-031A8F4CC526
// Assembly location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.dll
// XML documentation location: D:\GameLibrary\SteamLibrary\steamapps\common\Captain of Industry\Captain of Industry_Data\Managed\Mafi.xml

using eLxG93FZl5M3yHxFGb;
using System;

#nullable disable
namespace Mafi.Serialization
{
  /// <summary>
  /// Mark any property or field of a savable class / struct to make it ignored by the serializer generator. Note that
  /// properties without setters are ignored by default.
  /// </summary>
  [AttributeUsage(AttributeTargets.Property | AttributeTargets.Field | AttributeTargets.Event)]
  public sealed class DoNotSaveAttribute : Attribute
  {
    public readonly int? RemovedInSaveVersion;
    public readonly Option<Type> ResolveAfterLoad;

    public DoNotSaveAttribute(int removedInSaveVersion = 0, Type resolveAfterLoad = null)
    {
      MBiHIp97M4MqqbtZOh.RFLpSOptx();
      // ISSUE: explicit constructor call
      base.\u002Ector();
      this.RemovedInSaveVersion = removedInSaveVersion == 0 ? new int?() : new int?(removedInSaveVersion);
      this.ResolveAfterLoad = (Option<Type>) resolveAfterLoad;
    }
  }
}
