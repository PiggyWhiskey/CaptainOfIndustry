...
namespace Mafi
{
  public interface IEvent<T1, T2> : IEventNonSaveable<T1, T2>
  {
    /// <summary>
    ...
    /// </summary>
    void Add<TOwner>(TOwner owner, Action<T1, T2> action) where TOwner : class;

    void Remove<TOwner>(TOwner owner, Action<T1, T2> action) where TOwner : class;

    bool IsAdded<TOwner>(TOwner owner, Action<T1, T2> action) where TOwner : class;
  }
}
